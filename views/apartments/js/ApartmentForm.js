document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const payload = {
            ...
            tong_so_phong: Number(document.getElementById("tongSoPhong").value.trim()),
            so_dien_thoai: document.getElementById("soDienThoai").value.trim() || null,
            email: document.getElementById("email").value.trim() || null,
            ghi_chu: document.getElementById("ghiChu").value.trim() || null
        };

        try {
            const url = canHoId ? `/api/canhos/${canHoId}` : "/api/canhos";
    ...
    async function loadApartmentData(id) {
        try {
            const res = await fetch(`/api/canhos/${id}`);
    ...
    function validateForm() {
        ...
        if (!tongSoPhong.value.trim()) {
            ...
            showError(tongSoPhong, "errTongSoPhong", "Tổng số phòng phải lớn hơn hoặc bằng 1");
        ...
        if (phone.value.trim() && !phoneRegex.test(phone.value.trim())) {
            showError(phone, "errPhone", "Số điện thoại phải chứa 10-11 số và bắt đầu bằng số 0");
        ...
        if (!isValid) {
            ...
            document.querySelector(".validation-title span").textContent = errors.length;
        }
